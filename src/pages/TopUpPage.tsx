...
const fmtPrice = (n: number) =>
  Number.isFinite(n) ? n.toFixed(2).replace(/\.00$/, ".00") : "0.00";
...
const TopUpPage = () => {
  ...
  useEffect(() => {
    ...
    const line = ft?.line || cachedLines?.[0] || null;
    const msisdn = (line as any)?.msisdn || ft?.phone || "";
    if (msisdn) {
  ...
  const isValid = displayAmount >= 3 && isEmailValid && phone.length > 0;
  ...
  const handlePay = () => {
    ...
    toast({
      ...
      description: "El pago con tarjeta estará disponible muy pronto.",
    });
  };
