      ...
      {!showForm ? (
        ...
        <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-5 space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-white/60 text-xs">Hora de dormir</label>
            <Input
              type="datetime-local"
              value={startTime}
          ...
          <div className="space-y-2">
            <label className="text-white/60 text-xs">Hora de despertar</label>
            <Input
              type="datetime-local"
              value={endTime}
          ...
          <div className="space-y-2">
            <label className="text-white/60 text-xs">Calidad (1-5)</label>
            <div className="flex gap-2">
              {[1, 2, 3, 4, 5].map((v) => (
                ...
                >
                  <Star
                    className={`w-6 h-6 ${v <= quality ? "text-yellow-300 fill-yellow-300" : "text-white/30"}`}
                  />
                </button>
              ))}
            </div>
                  ...
                  {r.quality_rating && (
                    <div className="flex mt-1">
                      {[1, 2, 3, 4, 5].map((v) => (
                        <Star
                          key={v}
                          className={`w-3 h-3 ${v <= r.quality_rating! ? "text-yellow-300 fill-yellow-300" : "text-white/20"}`}
                        />
                      ))}
                    </div>
                  )}
